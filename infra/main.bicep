      ...
      ? [
          {
            ...
            value: (shouldDeployAzureOpenAIService)
              ? 'https://${keyVault.outputs.name}${environment().suffixes.keyvaultDns}/secrets/${azureOpenAi.outputs.cognitiveServicesKeySecretName}'
              : ''
...
module cognitiveSecret './shared/keyvault-cognitive-secret.bicep' =
  if (shouldDeployAzureOpenAIService) {
    ...
    params: {
      cognitiveServiceName: (shouldDeployAzureOpenAIService) ? azureOpenAi.outputs.name : ''
      cognitiveServiceResourceGroup: (shouldDeployAzureOpenAIService) ? azureOpenAi.outputs.resourceGroupName : ''
      keyVaultName: keyVault.outputs.name
      name: (shouldDeployAzureOpenAIService) ? azureOpenAi.outputs.cognitiveServicesKeySecretName : ''
    }
  }
